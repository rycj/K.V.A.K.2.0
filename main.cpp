...
int main(int, char **)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"DNDapp", WS_OVERLAPPEDWINDOW, 100, 100, (int)(1280 * main_scale), (int)(800 * main_scale), nullptr, nullptr, wc.hInstance, nullptr);
    ...
    // Setup scaling
    // ImGuiIO &io = ImGui::GetIO();    // Bake a fixed style scale. (until we have a solution for dynamic style scaling, changing this requires resetting Style + calling this again)
    ImGui::GetIO().FontGlobalScale = main_scale; // Set initial font scale. (using io.ConfigDpiScaleFonts=true makes this unnecessary. We leave both here for documentation purpose)
    ...
    // Main loop
    Constants UIConstants = Constants();
    Colours UIColours = Colours();
    int activeTab = 0;
    UIColours.updateColours(activeTab);
    ...
    mainMenu_flags |= ImGuiWindowFlags_NoBringToFrontOnFocus;
    tab_flags |= ImGuiWindowFlags_NoTitleBar;
    tab_flags |= ImGuiWindowFlags_NoMove;
    tab_flags |= ImGuiWindowFlags_NoResize;
    tab_flags |= ImGuiWindowFlags_NoCollapse;
    mainMenu_flags |= ImGuiWindowFlags_NoBackground;

    bool done = false;
    while (!done)
    {
        colors[ImGuiCol_WindowBg] = UIColours.windowBgCol;
        colors[ImGuiCol_Text] = UIColours.textCol;
        colors[ImPlotCol_PlotBorder] = UIColours.plotAxisCol;
        ...
        ImGui::NewFrame();
        UIConstants.updateUIConstants();
        /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        static bool use_work_area = true;
        ImGui::SetNextWindowPos(use_work_area ? UIConstants.main_viewport->WorkPos : UIConstants.main_viewport->Pos);
        ImGui::SetNextWindowSize(use_work_area ? UIConstants.main_viewport->WorkSize : UIConstants.main_viewport->Size);

        ImGui::Begin("Main window", NULL, mainMenu_flags);
        {
            for (int i = 0; i < 5; i++)
            {
                if (i > 0)
                    ImGui::SameLine();
                ImGui::PushID(i);
                ImGui::PushStyleColor(ImGuiCol_Button, UIColours.menuButtonColList[i]);
                ImGui::PushStyleColor(ImGuiCol_ButtonHovered, UIColours.menuButtonColList[i]);
                ImGui::PushStyleColor(ImGuiCol_ButtonActive, UIColours.menuButtonColList[i]);
                ImGui::PushStyleColor(ImGuiCol_Text, UIColours.menuTextColList[i]);
                if (ImGui::Button(UIConstants.tabList[i]))
                {
                    activeTab = i;
                    colors[ImGuiCol_WindowBg] = UIColours.windowBgCol;
                    UIColours.updateColours(i);
                }
        ...
        float clear_color[4];
        clear_color[0] = UIColours.menuCol.x;
        clear_color[1] = UIColours.menuCol.y;
        clear_color[2] = UIColours.menuCol.z;
        clear_color[3] = UIColours.menuCol.w;
